...
class train_model:
    ...
    def train(self):
        data =pd.read_csv('D:/Internship/Online/CodeClause/Personality_Prediction_Using_CV_Analysis/training_dataset.csv')  # in this line you have to add your Dataset path
        array = data.values
...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
  ...
  #  result.geometry('700x550')
    ...
    Label(result, text="Result of Personality Prediction by Rajesh Patil", foreground='green', bg='white', font=titleFont, pady=10, anchor=CENTER).pack(fill=BOTH)
    ...
    Label(result, text = str('{} : {}'.format("Email:","07rajeshpatil@gmail.com")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Mobile Number:", "7498862048")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Skills:", "Java, Python, Machine Learning")).title(), foreground='black', bg='white', anchor='w').pack(fill=BOTH)
